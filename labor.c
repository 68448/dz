...
{	
	...
	////появляется 9 округляем
	if ( lastFloat == 9 || lastFloat == 7 )
		lastInt++;
		
	
	//переписываем с типизацией
	result = (int) decimal;
...
int main (void)
{
	...
	// для минимальных знач
	int minValue = 100;
	int minCount = 0;

	////повторения мин значений
	int minRepeatX = 0;
	int minRepeatY = 0;
...
//массив минимальных координат
	...
	//С//считаем количество повторений
...
// сммотрим совпадениями с координатами минимума, если есть, то прерываем итерацию            for (tmp = 1; tmp < minCount * 2; tmp+= 2) 
		...
		{	
			...
			{	
				...
				while ( *(numbersString + currentSumbol) != '.')
				{
					...
					// если предыдущий символ не был запятой, значит не в начале строки, но возвожно это и начало
						...
						{	
							...
							{	
								...
								//запишем число в массив, умноженное на 10 в N степени, чтобы не потерять 
